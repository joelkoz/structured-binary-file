...
module.exports = class FixedRecordFile {
    ...
    /**
     * Retrieves and returns the first record in the file. To
     * retrieve the record after that, call getNext(). null is 
     * returned if there are no records in the file.
     */
    getFirst() {
        this.filePointer = 0;
        if (this.filePointer < this.header.recordCount) {
    ...
    /**
     * Retrieves and returns the NEXT record in the data file following
    ...
    /**
     * Returns the physical record number of the last record retrieved with
     * getFirst() or getNext(), or the physical record number of
     * a record added with appendRecord() (depending on which method was
     * called last)
     */
    ...
    /**
     * A convenience function that iterates over every record in the file, calling
     * fn(rec, recNum) with each iteration.
     * @param {function(record, recordNumber)} fn A function that does something with the record
     */
    forEach(fn) {
        var rec = this.getFirst();
        while (rec != null) {
            fn(rec, this.filePointer);
            rec = this.getNext();
        } // while
    }
